			...
			<div class="col-md-6 col-md-offset-3" id="post">
				...
				<div class="row">
					<c:if test="${ navName == 'HOME' }">
						<a href="DisplayOnePost?id=${aPostBean.postID}"> <img
				...
				<div class="row">
					<div class="col-xs-12" id="postLike">
						<c:if test="${ aPostBean.postLikeCheck == 0 }">
							<form action="<c:url value='InsertPostLike.do' />" method="POST"
								name="aPostLikeForm">
								...
								<c:if
									test="${aPostBean.postLikeBeans.size()>0 && aPostBean.postLikeBeans.size() <= 4}">
									<c:forEach varStatus="stVar" var="aPostLikeBean"
										items="${ aPostBean.postLikeBeans }">
								${aPostLikeBean.userName}&nbsp;&nbsp;
									</c:forEach>
								共${aPostBean.postLikeBeans.size()}人說這個讚 
								</c:if>
								<c:if test="${aPostBean.postLikeBeans.size()>4}">
								共${aPostBean.postLikeBeans.size()}人說這個讚 
								</c:if>
						...
						<c:if test="${ aPostBean.postLikeCheck == 1 }">
							<form action="<c:url value='DeletePostLike.do' />" method="POST"
								name="bPostLikeForm">
								<input type="hidden" id="postID" name="postID"
									value="${ aPostBean.postID }">
								<button type="submit" class="btn btn-default">
