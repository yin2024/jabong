...
public class TimeService {
    ...
    // 方法一、取得相對時間：
    public static String getRelativeTime(String absoluteTime) {
	...
	try {
	    Date old = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SS").parse(absoluteTime);

	    // 取得當下的時間
	    Date now = new Date();

	    // 由於Java處理時間是以毫秒計算，所以要除1000L將毫秒轉換成秒
	    long between = (now.getTime() - old.getTime()) / 1000L;
	    ...
	    if (day >= 365) {
		relativeTime = year + "年前";
	    } else if (day > 30) {// days<365 &&
		relativeTime = month + "個月前";
	    } else if (day >= 7) { // days<=30 &&
		relativeTime = week + "週前";
	    } else if (day >= 1) {
		relativeTime = day + "日前";
	    ...
	    System.err.println("TimeService類別轉換時間出錯");
	    e.printStackTrace();
	}
