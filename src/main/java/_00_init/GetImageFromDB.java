...
public class  GetImageFromDB extends HttpServlet {
	...
	try {
	    ...
	    if (type.equalsIgnoreCase("user")) { // 讀取eBook表格
		pstmt = conn.prepareStatement("SELECT userID, image FROM userImage WHERE userID = ?;");
	    } else if (type.equalsIgnoreCase("post")) { // 讀取eMember表格
		pstmt = conn.prepareStatement("SELECT postID, image FROM postImage WHERE postID = ?");
	    } else if (type.equalsIgnoreCase("group")) { // 讀取eMember表格
		pstmt = conn.prepareStatement("SELECT groupsID, image FROM groupsimage WHERE groupsID = ?");
	    }
